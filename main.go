...
func special(charName, charClass string) string {
	...
	switch charClass {
	case "warrior":
		skillName, power = "Выносливость", 80+25
	case "mage":
		skillName, power = "Атака", 5+40
	case "healer":
		skillName, power = "Защита", 10+30
	default:
		return "неизвестный класс персонажа"
	}
	return fmt.Sprintf("%s применил специальное умение `%s %d`", charName, skillName, power)
}
...
func choiseCharClass() string {
	...
	for approveChoice != "y" {
		...
		default:
			fmt.Println("Такого класса нет. Попробуй еще раз.")
			continue
		}
